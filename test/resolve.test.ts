...
describe("resolveUrl", () => {
  ...
  test("detects Vercel provider URL", () => {
    ...
    expect(resolveUrl().url).toBe("https://myapp-abc123.vercel.app")
  })

  test("falls back to localhost in development", () => {
    process.env.NODE_ENV = "development"
    expect(resolveUrl().url).toBe("http://localhost:3000")
  })

  test("uses PORT env var for localhost fallback", () => {
    process.env.NODE_ENV = "development"
    process.env.PORT = "4000"
    expect(resolveUrl().url).toBe("http://localhost:4000")
  })
  ...
  test("normalizes bare hostnames from providers", () => {
    ...
    expect(resolveUrl().url).toBe("https://myapp.com")
  })
  ...
  test("detects Vercel from VITE_VERCEL_ENV on client", () => {
    ...
    expect(resolveUrl().url).toBe("https://myapp-git-feat.vercel.app")
  })
})
